...
/* Get the comments string, add it to the comments array of
   pointers.  */
void 
ForComments(char **comments, size_t *buff_comments, char *str)
{
  ...
  /* If it is getting longer than the actual length, make the comments
     array longer: */
  if ( (strlen(*comments)+strlen(str)) >= *buff_comments) 
    {
...
/*  Get the first data string and see how many columns of data there
    are in it.  */
void 
CountCols(char *str, size_t *s1, double **table, size_t buff_num_rows)
{
...
void 
replacenan(struct ArrayInfo *intable, size_t col, char *strdata, 
        size_t *buff_num_replacements)
{
  int *temp_i_pt;
...
/* Knowing the number of columns, this function will read in each
   row.  */
void 
AddRow(struct ArrayInfo *intable, size_t *buff_num_rows, 
       size_t *buff_num_replacements, char *str)
{
  size_t num_cols=1, z_index;
  ...
  while (1)
	...
	replacenan(intable, num_cols-1, strdata, 
	 ...
	 abort the program and inform the user. */
	  ...
	  printf("%lu (starting from 1).\n",intable->s0+1);
	  printf("### --------------should have");
	  printf("%lu but has %lu.\n", intable->s1, num_cols);
	  exit(EXIT_FAILURE);
        }        
    }
...
readasciitable (const char *filename, struct ArrayInfo *intable)
{
  ...
  size_t buff_num_rows=BUFFER_NUM;
  size_t buff_comments=BUFFER_NUM;
  size_t buff_num_replacements=2*BUFFER_NUM; /* Has to be even */
